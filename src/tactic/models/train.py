...
def _apply_stats(b, mu, sd, device):
    ...
    return (torch.tensor(x, dtype=torch.float32, device=device),
            ...
            torch.tensor(b.y, dtype=torch.float32, device=device))


def _eval_loss(model, batches, mu, sd, taus_t, device):
    model.eval()
    tot, n = 0.0, 0
    with torch.no_grad():
        for b in batches:
            x, u, y = _apply_stats(b, mu, sd, device)
...
def train_one_seed(model_kind, batches_tr, batches_va, batches_te, mu, sd,
    ...
    for ep in range(epochs):
        ...
        sched.step()
        tr_loss = tot / max(n, 1)
        va_loss = _eval_loss(model, batches_va, mu, sd, taus_t, device)
...
def train(cfg: dict | None = None, model_kind: str = "f4", epochs: int = 25,
    ...
    (run_dir / "config.json").write_text(json.dumps(
        {"model": model_kind, "epochs": epochs, "seeds": seeds,
         ...
         "best_val_pinball": float(np.mean(val_losses))}, indent=2), encoding="utf-8")
